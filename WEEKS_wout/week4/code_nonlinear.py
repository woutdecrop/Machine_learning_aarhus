...
def plot_contour(w, phi, ax):
    ...
    for i in range(len(ys)):
        for j in range(len(xs)):
            ...
            img[i,j]= np.dot(w.reshape(-1,1).T,phi(np.array([x,y])).T)[0,0]
...
class LinRegClassifier():
    ...
    def fit(self, X, y):
        ...
        ### END CODE
        self.w =  np.linalg.pinv(X) @ y #formule p 85
